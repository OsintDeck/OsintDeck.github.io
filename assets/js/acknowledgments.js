// Acknowledgments Data — fotos locales en assets/img/autoridades/
const AUTORIDADES_IMG_BASE = "assets/img/autoridades/";
/** Cuando el nombre mostrado no coincide con el archivo (ej. ortografía del PNG) */
const AUTORIDADES_PHOTO_FILE = {
    "Santiago González Bellengeri": "Santiago González Bellenger.png",
};

function autoridadPhotoUrl(displayName) {
    const file = AUTORIDADES_PHOTO_FILE[displayName] || `${displayName}.png`;
    return AUTORIDADES_IMG_BASE + encodeURIComponent(file);
}
...
function renderAcknowledgments() {
    ...
    AUTHORITIES.forEach(person => {
        authoritiesGrid.appendChild(
            createCard({ ...person, photo: autoridadPhotoUrl(person.name) }, true)
        );
    });
    ...
    sortedProfessors.forEach(person => {
        professorsGrid.appendChild(
            createCard({ ...person, photo: autoridadPhotoUrl(person.name) }, false)
        );
    });
